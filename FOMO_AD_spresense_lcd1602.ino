#include <Visual_Anomaly_Detection_FOMO_AD__inferencing.h>
...
void loop()
{
...
#if EI_CLASSIFIER_HAS_VISUAL_ANOMALY == 1
    bool bb_found = result.visual_ad_grid_cells[0].value > 0;
    ...
    for (size_t ix = 0; ix < result.visual_ad_count; ix++) {
        ...
        // Check if the current cell is within the borders to be ignored
        if ((bb.x == 0 && bb.y >= 0 && bb.y <= 76) ||
            (bb.x == 76 && bb.y >= 0 && bb.y <= 76) ||
            (bb.y == 0 && bb.x >= 0 && bb.x <= 76) ||
            (bb.y == 76 && bb.x >= 0 && bb.x <= 76)) {
            continue; // Skip printing for these conditions
        }
        // Check for specific conditions
        if (bb.x == 19 && bb.y >= 19 && bb.y <= 57) {
            ei_printf("    LED_problem? (");
            ...
            lcd.print("Anomaly!");
            lcd.setCursor(0,1);
            lcd.print("LED problem?");
            anomaly_detected = true;
            continue; // Ensure the loop continues correctly
        }
       
        if (bb.x == 57 && bb.y == 57) {
            ei_printf("    right_button? (");
            ei_printf_float(bb.value);
            ei_printf(") [ x: %u, y: %u, width: %u, height: %u ]\n", bb.x, bb.y, bb.width, bb.height);
            lcd.clear();
            lcd.setCursor(0,0);
    ...
    if (!anomaly_detected) {
        ei_printf("   No_anomaly\n");
        lcd.clear();
        lcd.setCursor(0,0);
        lcd.print("No Anomaly");
        lcd.setCursor(0,1);
        lcd.print("OK");
    }
#endif
